...
const getProjects = async (req, res) => {
  try {
    ...
    const recordsPerPage = req?.body?.records
      ? parseInt(req?.body?.records)
      : 20;
    const query = techStack.length
      ? { techStack: { $in: techStack }, endDate: { $gt: new Date() } }
      : {};
...
const updateProjectDetails = async (req, res) => {
  ...
  const result = await db.collection(collections.project).updateMany(
    // get records with endDate less than 6 months old and ends in 6 months from today
    {
      endDate: {
        $gt: new Date(lessThanSixMonths), // greater than 6 months ago
    ...
    [
      {
        $set: {
          endDate: {
            $dateAdd: { startDate: "$endDate", unit: "year", amount: 3 },
          },
        },
      },
    ]
